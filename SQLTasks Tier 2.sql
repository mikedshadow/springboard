...
SELECT COUNT(f.name) AS 'Number with no charge'
FROM Facilities AS f
WHERE f.membercost = 0;
...
SELECT facid AS 'Id', name AS 'Facility', membercost AS 'Fee', monthlymaintenance AS 'Maintainance'
FROM Facilities
...
SELECT DISTINCT CONCAT(m.firstname, ' ', m.surname) AS membername, f.name AS court
FROM Facilities AS f
...
LEFT JOIN Facilities AS f ON f.facid = b.facid
LEFT JOIN Members AS m ON m.memid = b.memid
WHERE b.starttime LIKE '2012-09-14%' AND b.bookid IN(
    SELECT b.bookid 
    FROM Bookings AS b
    LEFT JOIN Facilities AS f ON f.facid = b.facid
...
GROUP BY facility HAVING revenue < 1000;

/* Q11: Produce a report of members and who recommended them in alphabetic surname,firstname order */

SELECT (m.surname || ', ' || m.firstname) AS member, (m1.firstname|| ' '|| m1.surname) AS recommender
FROM Members AS m
...
SELECT f.name AS facility, (m.surname || ', ' || m.firstname) AS member, COUNT(b.bookid) AS times_used
FROM Bookings AS b
...
SELECT strftime('%m', b.starttime) AS month, f.name AS facility, COUNT(b.bookid) AS times_used
FROM Bookings AS b
LEFT JOIN Facilities AS f ON b.facid = f.facid
LEFT JOIN Members AS m ON m.memid = b.memid 
WHERE b.memid != 0
